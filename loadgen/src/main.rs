...
fn gethostname() -> String {
    ...
    let mut buf = std::vec::from_elem(0i8, max_len);
    let ptr = buf.as_mut_ptr();
    let err = unsafe { libc::gethostname(ptr, max_len) };
    if err != 0 {
        println!("gethostname failed");
        return String::from("unknown");
    }

    let actual_len = buf.iter().position(|byte| *byte == 0).unwrap_or(max_len);
    let slice = unsafe { slice::from_raw_parts(buf.as_ptr() as *const u8, actual_len) };
    let hostname = String::from_utf8(slice.to_vec()).unwrap();
    hostname
}
...
fn process_result_final(
    ...
    let actual = packet_count as u64 * 1000_000_000 / duration_to_ns(last_send - first_send);
    ...
    if let OutputMode::File = sched.output {
        let hostname = gethostname();
        let filename = format!("/tmp/loadgen_latencies_epoch{}_{}.txt", step_idx, hostname);
        let mut f = File::create(filename).unwrap();

        writeln!(
            f,
            "Distribution,Target,Actual,Dropped,Never Sent,Start,StartTsc,Latency,Frequency"
        )
        .unwrap();
    ...
    if let OutputMode::Trace = sched.output {
        ...
        let filename = format!("/tmp/loadgen_trace_{}.txt", n);
        println!("Trace output: {}", filename);
        ...
        writeln!(f, "ActualStart,StartDelay,CompletionTime(us),ServerTSC").unwrap();
        for p in results.into_iter().filter_map(|p| p.trace).kmerge() {
            if let Some(completion_time) = p.completion_time {
                ...
                writeln!(
                    ...
                    duration_to_ns(completion_time) / 1000,
                    p.server_tsc,
                )
                .unwrap();
                ...
                writeln!(f, "{},-1,-1,-1", duration_to_ns(p.target_start)).unwrap();
            }
        }
        println!("");
    }
...
fn process_result(sched: &RequestSchedule, packets: &mut [Packet]) -> Option<ScheduleResult> {
    ...
    for p in packets.iter() {
        match (p.actual_start, p.completion_time) {
            (None, _) => never_sent += 1,
            (_, None) => dropped += 1,
            (Some(ref _start), Some(ref end)) => {
                *latencies.entry(duration_to_ns(*end) / 1000).or_insert(0) += 1
            }
        }
    }
...
fn run_client_worker(
    ...
    let socket = match live_mode_socket {
        ...
        _ => Arc::new(match tport {
            ...
            Transport::Fake => {
                // This is hack to initialize sandook related structures.
                proto.gen_req(0, &packets[0], &mut payload);
                backend.create_fake_connection(packets.len()).unwrap()
            }
        }),
    };
    ...
    let receive_thread = backend.spawn_thread(move || {
        ...
        for i in 0..receive_times.len() {
            match rproto.read_response(&socket2, &mut buf) {
                Ok((mut idx, start_tsc, total_tsc)) => {
                    if use_ordering {
                        idx = i;
                    }
                    receive_times[idx] = Some((Instant::now(), start_tsc, total_tsc));
                }
                Err(e) => {
                    match e.raw_os_error() {
    ...
    for (i, packet) in packets.iter_mut().enumerate() {
        ...
        if start_real == 0 {
            start_real = std::time::UNIX_EPOCH.elapsed().unwrap().as_micros();
            start_ns = duration_to_ns(start.elapsed());
        }
        packet.actual_start = Some(start.elapsed());
        if let Err(e) = (&*socket).write_all(&payloads[i][..]) {
    ...
    receive_thread
        ...
        .for_each(|(c, p)| {
            if let Some((_inst, start_tsc, total_tsc)) = c {
                (*p).completion_time = Some(Duration::from_micros(
                    total_tsc / unsafe { ffi::cycles_per_us as u64 },
                ));
                (*p).completion_server_tsc = Some(start_tsc);
            }
        });

    // First thread-is start from 100. This is to make a relation between first
    // packet actual start and wall clock time for plotting.
    if index == 100 {
        let filename = format!("/tmp/loadgen_start_time.txt");
        let mut f = OpenOptions::new()
            .create(true)
            .write(true)
            .append(true)
            .open(filename)
            .unwrap();
        writeln!(f, "start_ns,real_us").unwrap();
        writeln!(f, "{},{}", start_ns, start_real).unwrap();
    }
    ...
    let sockets: Vec<_> = (0..nthreads)
        ...
        .map(|i| {
            ...
            Arc::new(match tport {
                Transport::Tcp => backend.create_tcp_connection(Some(src_addr), addr).unwrap(),
                Transport::Udp => backend.create_udp_connection(src_addr, Some(addr)).unwrap(),
                Transport::Fake => backend.create_fake_connection(0).unwrap(),
            })
        })
        .collect();
    ...
    loop {
        ...
        schedules
            ...
            .map(|(i, sched)| {
                ...
                let r = process_result_final(sched, perthread, start_unix, sched_start, 0);
...
fn run_client(
    ...
    schedules
        ...
        .map(|(i, sched)| {
            let perthread = packets.iter_mut().filter_map(|p| p[i].take()).collect();
            let r = process_result_final(sched, perthread, start_unix, sched_start, index);
            sched_start += sched.runtime;
            r
        })
        .collect::<Vec<bool>>()
...
fn main() {
    let matches = App::new("Synthetic Workload Application")
        ...
        .arg(
            Arg::with_name("protocol")
                ...
                .long("protocol")
                .value_name("PROTOCOL")
                .possible_values(&["synthetic", "memcached", "dns", "reflex", "http", "sandook"])
        ...
        .arg(
            Arg::with_name("output")
                ...
                .possible_values(&["silent", "normal", "buckets", "trace", "file"])
    ...
    let runtime = Duration::from_millis(value_t!(matches, "runtime", u64).unwrap());
    ...
    let proto: Arc<Box<dyn LoadgenProtocol>> = match matches.value_of("protocol").unwrap() {
        ...
        "sandook" => Arc::new(Box::new(SandookProtocol::with_args(
            &matches,
            tport,
            distribution,
        ))),
    ...
    if let OutputMode::File = output {
        println!("Clearing old output files");
        let re = Regex::new(r"/tmp/loadgen_latencies_.*\.txt").unwrap();
    ...
    if let OutputMode::Trace = output {
        let filename = "/tmp/loadgen_trace.txt";
