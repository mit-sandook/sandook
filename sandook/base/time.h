...
// Returns the rdtsc timestamp in microseconds normalized by CPU's cycles/time
inline uint64_t AbsoluteMicroTime() { return rdtsc() / cycles_per_us; }

// Returns microseconds since the launch of the runtime.
...
class Duration {
 public:
  ...
  [[nodiscard]] constexpr bool IsZero() const { return duration_ == 0; }
...
class Time {
 public:
  ...
  // Now gets the current time since the launch of the runtime.
  static Time Now() { return Time(MicroTime()); }

  // Timeval converts the time to a Linux timeval.
  [[nodiscard]] constexpr timeval Timeval() const {
    return detail::us_to_timeval(time_);
  }
  // Timespec converts the time to a Linux timespec.
  [[nodiscard]] constexpr timespec Timespec() const {
    return detail::us_to_timespec(time_);
  }

  // Microseconds converts the time into microseconds.
  [[nodiscard]] constexpr uint64_t Microseconds() const { return time_; }
  // Milliseconds converts the time into milliseconds.
  [[nodiscard]] constexpr uint64_t Milliseconds() const {
    return time_ / kMilliseconds;
  }
