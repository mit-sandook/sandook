...
class TDigest {
 ...
 public:
  ...
  uint64_t quantile(Value q) {
    return static_cast<uint64_t>(quantile_(q));
  }

  // this returns a quantile on the t-digest
  Value quantile_(Value q) {
    if (haveUnprocessed() || isDirty()) process();
  ...
  void add(uint64_t x) { add_(static_cast<Value>(x)); }

  void add_(Value x) { add(x, 1); }
