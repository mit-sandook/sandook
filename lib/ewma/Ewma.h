...
class Ewma {
 public:
  ...
  double output = 0;
  /*
   * Smoothing factor, in range [0,1]. Higher the value - less smoothing (higher
   * the latest reading impact).
   */
  double alpha = 0;

  /*
   * Creates a filter without a defined initial output. The first output will be
   * equal to the first input.
   */
  explicit Ewma(double alpha) : alpha(alpha) {}

  /*
   * Creates a filter with a defined initial output.
   */
  Ewma(double alpha, double initialOutput)
      : output(initialOutput), alpha(alpha), hasInitial(true) {}

  void reset() { this->hasInitial = false; }
