...
function cleanup_linux() {
	...
	dirs_to_clean=""
	suffixes=$([0-9])
	dirs_to_clean="$(echo {/var/run,/tmp}/dpdk/spdk{,_pid}+${suffixes}) "
	if [[ -d $XDG_RUNTIME_DIR && $XDG_RUNTIME_DIR != *" "* ]]; then
		dirs_to_clean+="$(readlink -e assert_not_empty $XDG_RUNTIME_DIR/dpdk/spdk{,_pid}+${suffixes} || true) "
	fi
	...
	shopt -s extglob
	for fd_dir in $(echo /proc/+${suffixes}); do
